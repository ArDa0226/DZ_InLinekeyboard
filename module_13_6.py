...
async def send_calories(message, state):
    ...
    norm_cal_for_man = (10.0 * int(data['weight'])) + (6.25 * int(data['growth'])) - (5.0 * int(data['age'])) + 5
    norm_cal_for_woman = (10.0 * int(data['weight'])) + (6.25 * int(data['growth'])) - (5.0 * int(data['age'])) - 161
    await message.answer(f'Ваша норма калорий, необходимая для '
                         f'функцйонирования организма - {norm_cal_for_man} калорий.')
    await state.finish()
